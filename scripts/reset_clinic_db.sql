-- Schema for SilverClinic (minimal required tables)
...
CREATE TABLE IF NOT EXISTS case_profile(
    ...
    FOREIGN KEY(assessor_id) REFERENCES assessor(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS address(
    ...
    created_at TEXT NOT NULL,
    modified_at TEXT NOT NULL,
    FOREIGN KEY(user_key) REFERENCES client(id) ON DELETE CASCADE
);

-- Optional: indexes for common lookups
CREATE INDEX IF NOT EXISTS idx_client_email ON client(lower(trim(email)));
CREATE INDEX IF NOT EXISTS idx_assessor_normalized_email ON assessor(normalized_email);
CREATE INDEX IF NOT EXISTS idx_address_userkey ON address(user_key);
...
CREATE INDEX IF NOT EXISTS idx_family_physician_client ON family_physician(client_id);
CREATE INDEX IF NOT EXISTS idx_emergency_contact_client ON emergency_contact(client_id);
CREATE INDEX IF NOT EXISTS idx_case_profile_client ON case_profile(client_id);
