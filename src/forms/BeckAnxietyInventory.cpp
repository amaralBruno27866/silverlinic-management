    ...
    namespace Forms {
        ...
        bool BeckAnxietyInventory::isValidData() const {
            if (!isValidCaseProfileId(m_case_profile_id)) {
                utils::logStructured(utils::LogLevel::WARN, {"FORM","invalid_case_id","BAI", to_string(m_bai_id), {}}, "Invalid case profile ID");
                return false;
            }
            ...
            for (size_t i = 0; i < questions.size(); ++i) {
                if (!isValidQuestionValue(questions[i])) {
                    utils::logStructured(utils::LogLevel::WARN, {"FORM","invalid_q","BAI", to_string(m_bai_id), {}}, "Invalid value for question "+ to_string(i+1));
                    return false;
                }
            }
        ...
        void BeckAnxietyInventory::resetIdCounter() {
            bai_id_counter = 0;
            utils::logStructured(utils::LogLevel::INFO, {"FORM","reset_counter","BAI","",""}, "BAI ID counter reset");
        }
